...
export default function PortalPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/portal/${encodeURIComponent(token)}`).then(async (r) => {
      const j = await r.json();
      if (!r.ok) throw new Error(j.error ?? "not found");
      setData(j);
    }).catch((e) => setError((e as Error).message));
  }, [token]);
  if (error) return <Card className="p-6 text-sm">This portal link is not valid. Ask your RPG representative for a new one.</Card>;
