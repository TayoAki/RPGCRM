...
import { carrierName, fmtDateTime, gal, ppg, productName, supplierName, terminalName } from "@/lib/ops";
...
export default function BolsPage() {
  ...
  return (
    <Page>
      <PageHeader title="Bills of lading" description="Electronic BOLs from suppliers and terminals (sample feed in this MVP) plus manual entries. Each BOL is deduplicated and matched to a load; matched loads enter the billing workflow." actions={<><Button size="sm" variant="outline" onClick={() => setEntry(true)}>Enter BOL manually</Button><Button size="sm" disabled={!!busy} onClick={() => act("bols/pull").catch(() => undefined)}>Pull BOL feed</Button></>} />
